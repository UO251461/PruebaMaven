...
//	private ModeloNoEditable modeloCategorias;
	...
	/*
	 * Si la fecha que vas a comparar es anterior al argumento date, el mï¿½todo
	 * devolverï¿½ un valor menor a cero. Si la fecha que vas a comparar es
	 * posterior al argumento date, el mï¿½todo devolverï¿½ un valor mayor a cero.
	 * Si las fechas son iguales, el mï¿½todo devolverï¿½ un cero
	 */
	...
	//CATEGORIAS VAN SIEMPRE DE 18 A 200 Aï¿½OS
	private boolean noHuecosCategorias(){
		
		//MASCULINO INFERIOR DEBA SER 18
		if(categorias.get(0).getLimiteInferior() != 18){
			JOptionPane.showMessageDialog(this, "La primera categoria masculina debe empezar con 18 aï¿½os");
			return false;
		}
			for(int i=0;i<categorias.size();i++){
				...
				if(i!=categorias.size()-1){
					...
					if(categorias.get(i).getSexo() != categorias.get(i+1).getSexo()){
						if(categorias.get(i).getLimiteSuperior() != 200){
							JOptionPane.showMessageDialog(this, "La ultima categoria masculina debe acabar con 200 aï¿½os");
							return false;
						}
					}
				}
				if(i == categorias.size()-1 ){
					...
					if(categorias.get(i).getLimiteSuperior() != 200){
						JOptionPane.showMessageDialog(this, "La ultima categoria femenina debe acabar con 200 aï¿½os");
						return false;
					}
				}
			}
